...
if ~isfield(frames,'focal_length') || isempty(frames.focal_length)
    ...
    frames.focal_length = 719.5459; % for testing with the Bxx.jpg sequences
end
...
for frame=1:frames.length-1    
    ...
    if false % choose between different ways of getting E
...
for frame=1:frames.length-1
    ...
    Graph{frame}.denseRepError = sum(().^2,1);
